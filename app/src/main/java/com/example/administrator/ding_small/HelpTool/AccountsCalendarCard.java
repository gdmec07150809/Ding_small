...
import com.example.administrator.ding_small.AccountsSearchByCalendarActivity;
...
public class AccountsCalendarCard extends View {
    ...
    private Paint mTextPaint1; // 绘制绿色的画笔
    private Paint mTextPaint2; // 绘制橙色的画笔
    ...
    public AccountsCalendarCard(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    public AccountsCalendarCard(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public AccountsCalendarCard(Context context) {
        super(context);
        init(context);
    }

    public AccountsCalendarCard(Context context, OnCellClickListener listener) {
        super(context);
    ...
    private void init(Context context) {
        ...
        mCirclePaint.setColor(Color.parseColor("#FDE8D9")); // 红色圆形
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mDownX = event.getX();
                mDownY = event.getY();
             System.out.println("坐标："+mDownX+":"+mDownY);
                break;
    ...
    class Cell {
        ...
        public Cell(CustomDate date, State state, int i, int j) {
            ...
            this.i = i;
            this.j = j;
            this.content=date.day + "";
        }

        public void drawSelf(Canvas canvas) {
            switch (state) {
                case TODAY: // 今天
                    mTextPaint1.setColor(getResources().getColor(R.color.orange));
                    mTextPaint.setColor(Color.BLACK);
                    mTextPaint2.setColor(getResources().getColor(R.color.green));
//                    canvas.drawRect((float) (mCellSpace * (i)), (float) ((j) * mCellSpace),(float) (mCellSpace * (i))+150,(float) ((j) * mCellSpace)+150,
//                            mCirclePaint);
                    canvas.drawCircle((float) (mCellSpace * (i + 0.5)),
                            (float) ((j + 0.5) * mCellSpace)-10, (float) (mCellSpace / 2),
                            mCirclePaint);
                    break;
            ...
            canvas.drawText(content,
                    (float) ((i + 0.5) * mCellSpace - mTextPaint
                            .measureText(content) / 2), (float) ((j+0.55)
                            * mCellSpace-25), mTextPaint);
            mTextPaint1.setTextSize(14f);
            mTextPaint2.setTextSize(14f);
            canvas.drawText(text,
                    (float) ((i + 0.75) * mCellSpace - mTextPaint
                            .measureText(text) / 2), (float) ((j + 0.85)
                            * mCellSpace-(mTextPaint.measureText(text,0,1)/2)), mTextPaint1);
            canvas.drawText(text1,
                    (float) ((i + 0.75) * mCellSpace - mTextPaint
                            .measureText(text) / 2), (float) ((j + 0.6)
                            * mCellSpace-(mTextPaint.measureText(text,0,1)/2)), mTextPaint2);
        }
    }
